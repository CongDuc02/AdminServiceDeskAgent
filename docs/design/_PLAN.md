...
| ADR-005 · ADR-016 | Một lượt graph tiến sát hạn chót của lượt, hoặc sát shutdown delay — cửa sổ drain không còn phủ được lượt                                                               | **Phân phối** thời lượng một lượt chạy `orchestrator` trong tiến trình `api` — nhìn phần đuôi, không nhìn trung bình — đặt cạnh hạn chót của lượt (A-031) và shutdown delay đã cấu hình. Bản trước đặt cạnh giới hạn thời gian request (A-025); từ ADR-016 giới hạn đó chỉ cắt stream, không cắt lượt                                                                                                                        |
...
| ADR-015           | Lease của `stored_object` không còn dài hơn hẳn thời lượng upload một bản render — cửa sổ ghi đè của phương án (b) mở rộng (A-059)                                      | **Phân phối** thời lượng upload một bản render — nhìn phần đuôi — đặt cạnh độ dài lease đang cấu hình, **trên cùng một trục thời gian**; kèm số lần `render_integrity_check` trượt với `RENDER_CHECKSUM_MISMATCH`                                                                                                                                                                                                            |
| ADR-013 · A-050   | Proxy của Render gom đệm response dạng stream: `turn.progress` tới cùng lúc `turn.reply`, tín hiệu tới trễ                                                              | Một phép thử tổng hợp chạy **từ ngoài Render** mở `GET /signals` và đo thời gian từ lúc kết nối tới sự kiện `signal` đầu tiên — server phát sự kiện đó ngay khi kết nối (mục SSE của `05-api.md`) — đặt cạnh thời điểm server ghi log đã phát sự kiện đó. Gom đệm thì khoảng cách phía client lớn hơn hẳn phía server, hoặc sự kiện chỉ tới khi stream đóng. Chạy lần đầu **ngay khi có môi trường Render đầu tiên** (A-050) |
